...
// API configuration
const API_URL = process.env.REACT_APP_API_URL || "http://localhost:3001/api/addData";
...
const BecaForm = () => {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      formData.append('parentRUT', values.parentRUT);
      formData.append('address', values.address);
      formData.append('phone', values.phone);
      formData.append('email', values.email);
      formData.append('childrenCount', childrenCount);
      formData.append('parentRelationship', values.parentRelationship);
      ...
      values.children.forEach((child, index) => {
        if (child.document) {
          formData.append('document', child.document);
        }
      });
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.error || 'Error al enviar el formulario');
      }
    } catch (error) {
